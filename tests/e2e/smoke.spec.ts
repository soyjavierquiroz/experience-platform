...
test("smart Module 0 video remains stable after YouTube mounts",async({page})=>{await page.goto("/demo/sos");await page.getByRole("button",{name:"Ver el mensaje de Janny"}).click();await page.getByRole("button",{name:"Reproducir Antes de escribirle, vuelve a ti"}).click();await expect(page.locator(".smart-video-player iframe")).toHaveAttribute("src",/youtube-nocookie\.com\/embed\/zx48s7bSUGY/);await page.waitForTimeout(1500);await expect(page.getByRole("heading",{name:"No le escribas todavía."})).toBeVisible();await expect(page.getByRole("button",{name:"Volver al audio"})).toBeVisible();await expect(page.getByText("Algo no salió bien")).toHaveCount(0)});
...
test("public demo is fully navigable and keeps interactions locally",async({page})=>{
  ...
  await page.goto("/");await expect(page.getByRole("heading",{name:"Hola, María."})).toBeVisible();await expect(page.getByText("Siempre disponible",{exact:true})).toBeVisible();await expect(page.getByText("Día 1 de 7",{exact:true})).toHaveCount(0);await expect(page.getByRole("link",{name:"Ir directamente al Día 1"})).toHaveAttribute("href","/demo/dia/1");await page.getByRole("link",{name:"Comenzar con una P.A.U.S.A."}).click();await expect(page).toHaveURL(/\/demo\/sos$/);await expect(page.getByText("Módulo 0 · Antes de escribirle, vuelve a ti")).toBeVisible();await expect(page.getByLabel("Avatar de Janny")).toHaveText("JH");await expect(page.locator(".smart-video-player iframe")).toHaveCount(0);await page.getByRole("button",{name:"Ver el mensaje de Janny"}).click();await expect(page.getByRole("button",{name:"Reproducir Antes de escribirle, vuelve a ti"})).toBeVisible();await expect(page.locator(".smart-video-player iframe")).toHaveCount(0);await page.getByRole("button",{name:"Reproducir Antes de escribirle, vuelve a ti"}).click();const moduleVideo=page.locator(".smart-video-player iframe");await expect(moduleVideo).toHaveAttribute("src",/youtube-nocookie\.com\/embed\/zx48s7bSUGY/);const moduleVideoSrc=await moduleVideo.getAttribute("src");expect(moduleVideoSrc).toContain("enablejsapi=1");expect(moduleVideoSrc).toContain("playsinline=1");expect(moduleVideoSrc).toContain("controls=0");expect(moduleVideoSrc).toContain("rel=0");await page.getByRole("button",{name:"Volver al audio"}).click();
  ...
  await page.getByLabel("En este momento quiero escribirle porque…").fill("Necesito sentir claridad antes de actuar.");await page.getByRole("button",{name:"Ya hice la pausa"}).click();await page.getByRole("button",{name:"Ansiedad"}).click();await page.getByRole("slider",{name:"Intensidad inicial"}).fill("8");await page.getByRole("button",{name:"Calma"}).click();await page.getByRole("button",{name:"Continuar",exact:true}).click();
  await page.getByLabel("Lo que realmente pasó").fill("No respondió mi último mensaje.");await page.getByLabel("Lo que mi ansiedad está imaginando").fill("Nunca le importé.");await page.getByRole("button",{name:"Mi miedo"}).click();await page.getByRole("button",{name:"Continuar",exact:true}).click();await page.getByRole("button",{name:"Escribir sin enviar"}).click();await page.getByRole("button",{name:"Elegir esta acción"}).click();
  await page.getByLabel("Lo que yo quería decirle era…").fill("Quería decirte que te extraño.");await page.getByLabel(/Lo que realmente quería/).fill("Que todavía me eliges.");await page.getByRole("button",{name:"Guardar sin enviar"}).click();await expect(page.getByText("Tu emoción salió. El mensaje no fue enviado.")).toBeVisible();await page.reload();await expect(page.getByLabel("Lo que yo quería decirle era…")).toHaveValue(/te extraño/);await page.getByRole("button",{name:"Continuar",exact:true}).click();
  await page.getByRole("radio",{name:/Mi paz también/}).click();await page.getByRole("button",{name:"Continuar",exact:true}).click();await page.getByRole("slider",{name:"Intensidad final"}).fill("4");await page.getByRole("button",{name:/Puedo esperar/}).click();await page.getByRole("button",{name:"Cerrar mi P.A.U.S.A."}).click();await expect(page.getByRole("heading",{name:"No actuaste en automático."})).toBeVisible();const dayOneLink=page.getByRole("link",{name:/Continuar con el Día 1/});await expect(dayOneLink).toHaveAttribute("href","/demo/dia/1");const guide=await page.request.get("/media/mnle/00-sos/guia-modulo-0.pdf");expect(guide.ok()).toBe(true);expect(guide.headers()["content-type"]).toContain("pdf");expect(await page.getByRole("link",{name:"Descargar la guía completa"}).getAttribute("href")).toBe("/media/mnle/00-sos/guia-modulo-0.pdf");await dayOneLink.click();await expect(page.locator(".day-one-orientation")).toBeVisible();await expect(page.locator("iframe")).toHaveCount(0);expect(Number((await pool.query("select count(*) from sos_executions")).rows[0].count)).toBe(before);await pool.end();
  await page.goto("/demo/ruta");await expect(page.getByText("Módulo 0 · P.A.U.S.A.")).toBeVisible();await expect(page.getByRole("link",{name:"Siempre disponible"})).toHaveAttribute("href","/demo/sos");await page.getByRole("link",{name:"Perfil"}).click();await expect(page.getByRole("heading",{name:"María"})).toBeVisible();

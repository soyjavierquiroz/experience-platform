...
test("premium dashboard is responsive, accessible and tenant-driven",async({page})=>{
  ...
  await expect(page.getByRole("link",{name:/Empezar el Día 1/})).toHaveAttribute("href","/app/dia/1");
  ...
  for(const [width,height] of [[768,1024],[1440,1000]] as const){await page.setViewportSize({width,height});expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(width);await expect(page.getByRole("link",{name:/Empezar el Día 1/})).toBeVisible();}
  ...
  await expect(page.getByRole("link",{name:/Empezar el Día 1/})).toBeVisible();await page.getByRole("link",{name:/Empezar el Día 1/}).focus();expect(await page.getByRole("link",{name:/Empezar el Día 1/}).evaluate(element=>getComputedStyle(element).outlineStyle)).not.toBe("none");

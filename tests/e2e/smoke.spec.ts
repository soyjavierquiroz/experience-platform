...
test("public demo is fully navigable and keeps interactions locally",async({page})=>{
  ...
  await page.goto("/");await expect(page.getByRole("heading",{name:"Hola, María."})).toBeVisible();await expect(page.getByRole("link",{name:"Ir directamente al Día 1"})).toHaveAttribute("href","/demo/dia/1");await page.getByRole("link",{name:"Comenzar con una P.A.U.S.A."}).click();await expect(page).toHaveURL(/\/demo\/sos$/);await expect(page.getByText("Módulo 0 · Antes de escribirle, vuelve a ti")).toBeVisible();
  ...
  await page.getByRole("radio",{name:/Mi paz también/}).click();await page.getByRole("button",{name:"Continuar"}).click();await page.getByRole("slider",{name:"Intensidad final"}).fill("4");await page.getByRole("button",{name:/Puedo esperar/}).click();await page.getByRole("button",{name:"Cerrar mi P.A.U.S.A."}).click();await expect(page.getByRole("heading",{name:"No actuaste en automático."})).toBeVisible();const dayOneLink=page.getByRole("link",{name:/Continuar con el Día 1/});await expect(dayOneLink).toHaveAttribute("href","/demo/dia/1");const guide=await page.request.get("/media/mnle/00-sos/guia-modulo-0.pdf");expect(guide.ok()).toBe(true);expect(guide.headers()["content-type"]).toContain("pdf");expect(await page.getByRole("link",{name:"Descargar la guía completa"}).getAttribute("href")).toBe("/media/mnle/00-sos/guia-modulo-0.pdf");await dayOneLink.click();const dayOneVideo=page.locator('iframe[title="Antes de enviar: mira el ciclo completo"]');await expect(dayOneVideo).toHaveAttribute("src","https://www.youtube-nocookie.com/embed/zx48s7bSUGY");await expect(dayOneVideo).toHaveAttribute("loading","lazy");await expect(dayOneVideo).toHaveAttribute("allowfullscreen","");expect(Number((await pool.query("select count(*) from sos_executions")).rows[0].count)).toBe(before);await pool.end();

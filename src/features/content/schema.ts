...
export const contentAssetSchema=z.object({schemaVersion,key:assetRef,tenantSlug:slug,category:z.enum(["video","audio","image","document"]),provider:z.enum(["youtube","bunny","vimeo","wistia","file"]).optional(),providerMediaId:z.string().optional(),providerMetadata:z.record(z.string(),z.unknown()).default({}),title:z.string(),posterAssetRef:assetRef.optional(),aspectRatio:aspectRatioSchema.default({type:"3:4"}),durationSeconds:z.number().int().positive().optional(),captionsAssetRef:assetRef.optional(),sourceUrl:z.string().refine(value=>value.startsWith("/")||value.startsWith("https://"),"solo ruta local o HTTPS").optional(),mediaType:z.string().regex(/^[\w.+-]+\/[\w.+-]+$/).optional(),sizeBytes:z.number().int().positive().optional(),status:z.enum(["placeholder","ready","disabled"])}).superRefine((asset,ctx)=>{if(asset.category==="video"&&!asset.provider)ctx.addIssue({code:"custom",message:"video requiere provider"});if(asset.provider==="bunny"&&asset.status==="ready"&&(!asset.providerMetadata.libraryId||!asset.providerMediaId))ctx.addIssue({code:"custom",message:"Bunny requiere libraryId y videoId"});});

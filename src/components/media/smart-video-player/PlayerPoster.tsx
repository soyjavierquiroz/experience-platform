...
export function PlayerPoster({creator,title,onPlay}:{creator:CreatorMetadata;title:string;onPlay:()=>void}){return <button type="button" className="smart-video-poster" onClick={onPlay} aria-label={`Reproducir ${title}`}><span className="poster-light" aria-hidden="true"/><span className="poster-play"><PlayIcon/></span><span className="poster-copy"><small>Un mensaje de tu creadora</small><strong>{title}</strong><b>{creator.displayName}</b><span>{creator.role}</span></span><CreatorAvatar creator={creator}/></button>}

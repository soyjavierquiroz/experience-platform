...
export function CreatorAvatar({creator}:{creator:CreatorMetadata}){const initials=creator.displayName==="Janny"?"JH":creator.displayName.split(/\s+/).map(word=>word[0]).join("").slice(0,2).toUpperCase();return <span className="creator-avatar">{creator.avatarUrl?<Image unoptimized src={creator.avatarUrl} width={52} height={52} alt={`Janny, ${creator.role??"creadora"}`}/>:<span aria-label={`Avatar de ${creator.displayName}`}>{initials}</span>}</span>}

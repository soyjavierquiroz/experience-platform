...
export const auth=betterAuth({
  ...
  plugins:[emailOTP({otpLength:6,expiresIn:300,allowedAttempts:3,storeOTP:"hashed",rateLimit:{window:60,max:5},generateOTP:()=>process.env.EMAIL_PROVIDER==="test"&&process.env.ALLOW_TEST_EMAIL_PROVIDER==="true"?(process.env.TEST_OTP_CODE??"731946"):undefined,sendVerificationOTP:async({email,otp})=>sendOtpEmail(email,otp)})],
